...
export default function IDCard({ member }: { member: any }) {
  ...
  const handleDownload = async (e: React.MouseEvent) => {
    ...
    const frontNode = document.getElementById(`front-${member.employeeId}`);
    const backNode = document.getElementById(`back-${member.employeeId}`);
    ...
    try {
      ...
      if (ctx) {
        ...
        const formattedName = member.name.replace(/\s+/g, "_");
  ...
  return (
    <div
      className="group relative w-70 h-111.25 perspective-[1000px] cursor-pointer shrink-0 mx-auto md:mx-0"
      onClick={() => setIsFlipped(!isFlipped)}
    >
      {/* ADDED: absolute inset-0 to prevent layout shifting during 3D flip */}
      <div
        className={`absolute inset-0 w-full h-full transition-transform duration-700 transform-3d lg:group-hover:transform-[rotateY(180deg)] ${
        ...
        >
          ...
          <div className="mt-6 flex flex-col z-20 h-full">
            ...
            <p className="text-[7px] text-center text-black/40 dark:text-white/40 mt-3 tracking-wider">
              <span className="uppercase"> If found, please return to</span>{" "}
              <span
